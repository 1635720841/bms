    ...
    <div class="bms-device-content">
      <div class="bms-card">
        <div class="bms-card__head">
          ...
          <ProductionManageActions :has-selection="selectedRows.length > 0"
            :selected-bms-ids="selectedRows.map(it => it.bmsId)" @refresh="GetData" />
        </div>

        <!-- 库存切换：全部 / 生产库存 / 维修库存（仅在有库存配置时展示） -->
        <div v-if="hasStockConfig" class="bms-stock-switch">
          <el-button :type="stockViewMode === 'all' ? 'primary' : 'default'" plain
            @click="handleStockViewModeChange('all')">
            全部
          </el-button>
          <el-button :type="stockViewMode === 'production' ? 'primary' : 'default'" plain
            @click="handleStockViewModeChange('production')">
            生产库存
          </el-button>
          <el-button :type="stockViewMode === 'repair' ? 'primary' : 'default'" plain
            @click="handleStockViewModeChange('repair')">
            维修库存
          </el-button>
        </div>

        <TableSearch :form-item="searchItems" page-name="productionManage" @query-btn="handleSearch"
          @reset-btn="handleReset" />

        <pageTable :page="page" :data="list" :columns="columns" :loading="loading" rowkey="bmsId"
          :height="`${hasStockConfig ? 'calc(100vh - 380px)' : 'calc(100vh - 350px)'}`" @GetData="GetData"
          @selectionChange="handleSelectionChange">
          <template #cz="{ row }">
            <el-button link class="bms-op-link" @click="handleRowTransfer(row)">
              设备调拨
            </el-button>
            <el-dropdown trigger="click" popper-class="bms-prod-dropdown"

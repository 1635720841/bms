...
import SwitchTab from "./batchConfig/SwitchTab.vue";
...
const advancedConfigTabs: TabConfig[] = [
  ...
  // { key: 'dischargeSwitch', title: '放电开关', desc: '控制放电 MOS 开关状态', component: SwitchTab, defaultValue: 1, apiPath: '/bms/api/set/params/funCtrl', configName: '放电开关配置', validateMsg: '请选择放电开关状态', initForm: (tab, p) => { tab.form.op = (Number(p["discharge"] ?? 1) === 0 ? 0 : 1) as 0 | 1; }, getPayload: (tab) => ({ discharge: tab.getPayload().op }) },
  // { key: 'blindChargeSwitch', title: '盲充开关', desc: '控制盲充功能开关状态', component: SwitchTab, defaultValue: 1, apiPath: '/bms/api/set/params/funCtrl', configName: '盲充开关配置', validateMsg: '请选择盲充开关状态', initForm: (tab, p) => { tab.form.op = (Number(p["blindChg"] ?? 1) === 0 ? 0 : 1) as 0 | 1; }, getPayload: (tab) => ({ blindChg: tab.getPayload().op }) },
...
<template>
  <el-dialog
    ...
    destroy-on-close
    append-to-body
    class="param-config-dialog param-config-dialog-light"
    @update:model-value="emit('update:modelValue', $event)"
  >
    <div v-loading="loading || loadingParams || refreshingAfterSubmit" :element-loading-text="refreshingAfterSubmit ? '配置下发成功，5秒后自动刷新...' : ''" class="param-dialog">
        ...
        <el-form label-width="110px" class="form">
          <div class="main-tabs">
            <div
              ...
              :key="tab.name"
              :class="['main-tab-item', { active: activeMainTab === tab.name }]"
            ...
            <div class="card-grid-basic">
              <div v-for="config in basicConfigTabs" :key="config.key" class="param-card-item">
                ...
                <div class="param-card-item__body">
                  <component
                    :is="config.component"
                    :ref="(el: any) => { if (el) tabRefs[config.key] = el }"
...
.param-card-item__body {
  flex: 1;
  background: #f8fafc; /* 浅灰底色，与白色输入框形成对比 */
  border-radius: 8px;
  padding: 12px;
}
...
:deep(.el-textarea__inner) {
  ...
  box-shadow: 0 0 0 1px #e2e8f0 inset !important; /* 极细的淡灰边框 */
  border-radius: 8px;
  padding: 4px 12px;
...
:deep(.el-input.is-disabled .el-input__wrapper) {
  ...
  .el-input__inner {
    ...
    -webkit-text-fill-color: #000000 !important; /* 兼容 Safari */
  }
}
</style>

...
import geoJsonData from "@/utils/json/100000_full.json";
...
const emit = defineEmits<{
  (e: "provinceClick", provinceName: string): void;
}>();
...
function renderDeviceMap(list: DeviceRegionItem[]) {
  ...
  const geoJson = cachedGeoJson.value;
  if (!chart || !geoJson) return;
  const regionCode = "china_3d_pro";
  const option = buildDeviceMapOption(regionCode, list ?? [], geoJson);
...
function buildDeviceMapOption(
  ...
  const centerMapNormalized: Record<string, [number, number]> = {};
  geoJson.features.forEach(feature => {
    const props = feature.properties;
    const point = props.centroid ?? props.center;
    if (props.name && point) {
      centerMap[props.name] = point;
      const key = normalizeProvinceName(props.name);
      if (key) centerMapNormalized[key] = point;
    }
  });

  const scatterData = list
    .map(item => {
      const center = centerMap[item.name] ?? centerMapNormalized[normalizeProvinceName(item.name)];
  ...
  const topScatter = [...scatterData].sort((a, b) => b.value[2] - a.value[2]).slice(0, 6);
  const flyLines = topScatter.map(item => ({
    coords: [shenzhenCoord, [item.value[0], item.value[1]]],
    value: item.value[2]
  }));

  // 兼容：外部数据可能是 “广东/广东省/广东市/广西” 等不同命名
  const valueMap = new Map<string, number>(list.map(i => [i.name, i.value]));
  const valueMapNormalized = new Map<string, number>();
  list.forEach(i => {
    const key = normalizeProvinceName(i.name);
    if (!key) return;
    valueMapNormalized.set(key, (valueMapNormalized.get(key) ?? 0) + i.value);
  });

  const resolveValueByRegionName = (regionName: string): number | undefined => {
    const direct = valueMap.get(regionName);
    if (typeof direct === "number") return direct;
    const normalized = normalizeProvinceName(regionName);
    if (!normalized) return undefined;
    return valueMapNormalized.get(normalized);
  };
  ...
  const mapData = geoJson.features
    ...
    .map(name => {
      const value = resolveValueByRegionName(name);
  ...
  const layerCount = 5; // 增加到10层，厚度更饱满
  ...
  const option: EChartsOption = {
    ...
    tooltip: {
      ...
      borderWidth: 1,
      padding: [10, 15],
      textStyle: { color: "#e6edf3", fontSize: 13, fontFamily: "monospace" },
      formatter(params) {
        const p = params as { name: string; data?: { value?: unknown } };
        if (!p.data) return p.name;

        const rawValue = p.data.value;
        const count =
          typeof rawValue === "number"
            ? rawValue
            : Array.isArray(rawValue) && typeof rawValue[2] === "number"
              ? rawValue[2]
              : 0;
        ...
        return `
          ...
          <div><span style="color:#8ab4f8;">设备数量：</span><span style="color:#e6edf3; font-size: 16px; font-weight: bold;">${count || 0}</span></div>
          <div><span style="color:#8ab4f8;">区域占比：</span>${percent && Number(percent) > 0 ? `${percent}%` : "0.0"}%</div>
        `;
      }
    },
    visualMap: {
    ...
    series: [
      {
        ...
        label: {
          ...
          rich: {
            p: {
              // 文字颜色：改为略偏暗的浅色，并加深描边，提高在亮色区域上的可读性
              color: "#e6edf3",
              fontSize: 11,
              ...
              borderWidth: 0,
              padding: [0, 0],
              shadowColor: "rgba(1, 9, 21, 0.2)",
              shadowBlur: 2,
              textBorderColor: "rgba(1, 9, 21, 0.9)",
              textBorderWidth: 2
            }
          }
        },
      ...
      // 🌟 核心枢纽：使用醒目的金/橙色对比，突出中心
      {
        ...
        data: [{ name: "深圳", value: [shenzhenCoord[0], shenzhenCoord[1], 1] }]
      },
      // 🌟 普通节点：呼吸灯点位
      {
        name: "终端节点",
...
async function initDeviceMap() {
  ...
  try {
    ...
    // geoJsonData 是模块单例；这里会对 features/coordinates 做清洗与坐标变换，
    // 必须先深拷贝，否则“刷新当前页/重新挂载”会累计变换，导致港澳越变越大。
    const cloneDeep = <T,>(input: T): T => {
      const sc = (globalThis as unknown as { structuredClone?: <U>(v: U) => U }).structuredClone;
      if (typeof sc === "function") return sc(input);
      return JSON.parse(JSON.stringify(input)) as T;
    };

    const geoJson = cloneDeep(geoJsonData) as MapGeoJSON;
    ...
    cachedGeoJson.value = geoJson;

    renderDeviceMap(props.regionList);
  } catch (error) {
...
// 点击省份触发“省 -> 市”钻取（等待 chart 初始化完成后再绑事件）
watch(
  () => mapChartInstance.value,
  chart => {
    if (!chart) return;
    chart.off("click");
...
.map-wrapper {
  --hud-gap: clamp(8px, 0.8vw, 10px);
  --hud-size: clamp(16px, 1.4vw, 20px);
  --hud-border: clamp(1px, 0.2vw, 2px);
  flex: 1;
  min-height: 0;
  position: relative;
  background-color: #010915; // 深空暗色背景，凸显发光元素
  padding: var(--hud-gap); // 给角落装饰留出空间
}
...
.hud-corner {
  ...
  width: var(--hud-size);
  height: var(--hud-size);
  border: var(--hud-border) solid #00eaff;
  pointer-events: none;
  z-index: 10;
  opacity: 0.8;
  ...
  &.top-left {
    top: var(--hud-gap); left: var(--hud-gap);
    border-right: none; border-bottom: none;
  }
  &.top-right {
    top: var(--hud-gap); right: var(--hud-gap);
    border-left: none; border-bottom: none;
  }
  &.bottom-left {
    bottom: var(--hud-gap); left: var(--hud-gap);
    border-right: none; border-top: none;
  }
  &.bottom-right {
    bottom: var(--hud-gap); right: var(--hud-gap);

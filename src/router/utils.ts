...
function filterNoPermissionTree(data: RouteComponent[]) {
  ...
  const newTree = cloneDeep(data).filter((v: any) => {
    // 如果路由有 bmsAuth 标识，使用 BMS 权限检查
    if (v.meta?.bmsAuth && Array.isArray(v.meta.bmsAuth)) {
      return checkBmsPermission(v.meta.bmsAuth);
    }
    // 否则使用原来的 roles 检查
...
function getTopMenu(tag = false): menuType {
  ...
  if (!wholeMenus || wholeMenus.length === 0 || !wholeMenus[0]?.children || wholeMenus[0].children.length === 0) {
    return null;
  }

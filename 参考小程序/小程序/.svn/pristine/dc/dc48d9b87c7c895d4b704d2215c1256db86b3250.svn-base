...
Page({
  ...
  queryCfgSelections() {
    ...
    let url = '/bms/api/mng/xsrv/list'

    http.post(url, params, {}, true).then(res => {
      console.log('查服务器成功', res);
      if (res.errno === 0) {
        //测试数据
        let cfgs = res.data.xsrv_list
  ...
  onSectionChange(e) {
    ...
    this.setData({
      selectedSection: that.data.xsrList[index]
    });
  ...
  srvSubmit() {
    ...
    let params = {
      ...
      "offlineTask":that.data.offlineTask
     }

    if (!!that.data.selectedSection) { // 有选择目的地
      params.params = {
        "xuesrv":that.data.selectedSection.value
      }
    ...
    let url = '/bms/api/set_batch/params/server'
    http.post(url, params, {}, true).then(res => {
      console.log('主服务配置成功', res);
      if (res.errno === 0) {
        
        let content = "已成功提交后台处理"  
        wx.showModal({
          content: content,
          confirmText: '继续配置',
          cancelText: '返回',

// pages/deviceManager/deviceEntry/deviceEntry.js
const app = getApp();
...
Page({
  ...
  // 提交
  onConfirmEntry: util.throttle(function () {
    console.log("onConfirmEntry")
    let that = this;
    if (!that.data.devString) {
      wx.showToast({
        title: '请先输入设备编码',
  ...
  entryDeviceList() {
    ...
    // let str = that.data.devString.replaceAll('，', ',')
    // let devList = str.split(",");
    ...
    http.post('/bms/api/mng/dev/add', params, {}, true).then(res => {
      console.log('录入设备成功', res);
      if (res.errno === 0) {
        let count = res.data.add_cnt;
        let content = "提交录入" + list.length + "个设备，录入成功" + count + "个设备"
